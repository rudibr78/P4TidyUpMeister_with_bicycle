...
P4TidyUpMeister.isEventAdded = false
...
P4TidyUpMeister.inventoryItems = {}
...
P4TidyUpMeister.temp.inventoryItems = {}
...
P4TidyUpMeister.initOption = function()
	...
	P4TidyUpMeister.options.EnableFishing = options:addTickBox("EnableFishing", getText("UI_P4TidyUpMeister_Options_EnableFishing_Name"), true, getText("UI_P4TidyUpMeister_Options_EnableFishing_Tooltip"))
end
...
P4TidyUpMeister.setDoNotReequip = function()
	...
	if not modData.P4TidyUpMeister then
		...
		P4TidyUpMeister.player:transmitModData()
	end
...
P4TidyUpMeister.isBicycleItem = function(item)
	if not item then return false end

	local itemType
	if item.getType then itemType = item:getType() end
	if itemType == "Bicycle" or itemType == "Bicycle_RedStreet" then return true end

	local fullType
	if item.getFullType then fullType = item:getFullType() end
	return fullType == "Bicycle.Bicycle" or fullType == "Bicycle.Bicycle_RedStreet"
end

P4TidyUpMeister.isBicycleAction = function(action)
	if not action then return false end
	if action.bicycleMount then return true end

	local actionType = action.Type
	return actionType == "BicycleHopOnAction" or actionType == "BicycleDismountAction" or actionType == "BicycleThrowOverFenceAction"
end

P4TidyUpMeister.hasQueuedBicycleAction = function(player)
	if not player or not ISTimedActionQueue or not ISTimedActionQueue.getTimedActionQueue then return false end

	local queue = ISTimedActionQueue.getTimedActionQueue(player)
	if not queue then return false end
	if P4TidyUpMeister.isBicycleAction(queue.current) then return true end

	if queue.queue then
		for i=1,#queue.queue do
			if P4TidyUpMeister.isBicycleAction(queue.queue[i]) then return true end
		end
	end
...
P4TidyUpMeister.shouldSkipForBicycle = function(player, action, item)
	if P4TidyUpMeister.isBicycleAction(action) or P4TidyUpMeister.isBicycleItem(item) then return true end
	if P4TidyUpMeister.isBicycleItem(P4TidyUpMeister.primaryItem) or P4TidyUpMeister.isBicycleItem(P4TidyUpMeister.secondaryItem) then return true end
	if not player then return false end

	if player.getVariableBoolean then
		if player:getVariableBoolean("BicycleActive") or player:getVariableBoolean("Bicycle_Riding") or player:getVariableBoolean("Bicycle_MountPending") or
		player:getVariableBoolean("Dismounting") or player:getVariableBoolean("droppingBicycle") or player:getVariableBoolean("Bicycle_Stopping") then
			return true
		end
	end

	if player.getPrimaryHandItem and P4TidyUpMeister.isBicycleItem(player:getPrimaryHandItem()) then return true end
	if player.getSecondaryHandItem and P4TidyUpMeister.isBicycleItem(player:getSecondaryHandItem()) then return true end

	return P4TidyUpMeister.hasQueuedBicycleAction(player)
end
...
function ISWorldObjectContextMenu.equip(...)
	...
	local args = {...}
	local item = args[3]
	if not P4TidyUpMeister.shouldSkipForBicycle(P4TidyUpMeister.player, nil, item) then
...
-- [SET] Common
P4TidyUpMeister.ISFirearmRadialMenu_sharpenWeapon = ISFirearmRadialMenu.sharpenWeapon
function ISFirearmRadialMenu.sharpenWeapon(...)
	P4TidyUpMeister.set(P4TidyUpMeister.player)
	P4TidyUpMeister.ISFirearmRadialMenu_sharpenWeapon(...)
end

-- [SET] Craft
P4TidyUpMeister.ISInventoryPaneContextMenu_OnNewCraft = ISInventoryPaneContextMenu.OnNewCraft
function ISInventoryPaneContextMenu.OnNewCraft(...)
	P4TidyUpMeister.set(P4TidyUpMeister.player)
	P4TidyUpMeister.ISInventoryPaneContextMenu_OnNewCraft(...)
end

-- [SET] Craft
P4TidyUpMeister.ISWidgetHandCraftControl_startHandcraft = ISWidgetHandCraftControl.startHandcraft
function ISWidgetHandCraftControl:startHandcraft(...)
	P4TidyUpMeister.set(P4TidyUpMeister.player)
	return P4TidyUpMeister.ISWidgetHandCraftControl_startHandcraft(self, ...)
end
...
function ISFitnessAction:new(character, exercise, timeToExe, exeData, exeDataType)
	...
	return P4TidyUpMeister.ISFitnessAction_new(self, character, exercise, timeToExe, exeData, exeDataType)
end
...
function AIAttachLureAction:new(character, rod, lure)
	if P4TidyUpMeister.options.EnableFishing.value then
		P4TidyUpMeister.set(P4TidyUpMeister.player)
	end
	return P4TidyUpMeister.AIAttachLureAction_new(self, character, rod, lure)
end
...
function AIRemoveLureAction:new(character, rod)
	...
	return P4TidyUpMeister.AIRemoveLureAction_new(self, character, rod)
end

P4TidyUpMeister.ISInventoryTransferAction_new = ISInventoryTransferAction.new
function ISInventoryTransferAction:new(character, item, srcContainer, destContainer, time)
	if not P4TidyUpMeister.shouldSkipForBicycle(character, nil, item) then
		P4TidyUpMeister.setTransferredItem(item)
	end
	return P4TidyUpMeister.ISInventoryTransferAction_new(self, character, item, srcContainer, destContainer, time)
end

P4TidyUpMeister.ISInventoryTransferAction_start = ISInventoryTransferAction.start
function ISInventoryTransferAction:start()
	if not P4TidyUpMeister.shouldSkipForBicycle(self.character, self, self.item) then
...
function ISBaseTimedAction:stop()
	...
	if P4TidyUpMeister.player == self.character then
		if P4TidyUpMeister.shouldSkipForBicycle(self.character, self) then
...
P4TidyUpMeister.OnPlayerUpdate = function(player)
	...
	if P4TidyUpMeister.needsReequip and P4TidyUpMeister.shouldSkipForBicycle(player) then
		P4TidyUpMeister.reset()
	end
	...
	if (not P4TidyUpMeister.needsReequip) and (not player:hasTimedActions()) then
		P4TidyUpMeister.floorItems = {}
		P4TidyUpMeister.worldRotations = {}
		Events.OnPlayerUpdate.Remove(P4TidyUpMeister.OnPlayerUpdate)
		P4TidyUpMeister.isEventAdded = false
	end
end
...
P4TidyUpMeister.set = function(player, ignoreStop)
	if P4TidyUpMeister.shouldSkipForBicycle(player) then return end
	...
	if (not P4TidyUpMeister.needsReequip) and (not P4TidyUpMeister.doNotReequip) then
		...
		for i=0, inventoryItems:size()-1 do
			...
			P4TidyUpMeister.inventoryItems[inventoryItem] = true
		end
		...
		if not P4TidyUpMeister.isEventAdded then
			Events.OnPlayerUpdate.Add(P4TidyUpMeister.OnPlayerUpdate)
			P4TidyUpMeister.isEventAdded = true
		end
	end
end

P4TidyUpMeister.setTransferredItem = function(item)
	if P4TidyUpMeister.shouldSkipForBicycle(P4TidyUpMeister.player, nil, item) then return end
	...
	if (not P4TidyUpMeister.doNotReequip) and item then
		...
		if srcContainer then
			...
			elseif P4TidyUpMeister.temp.needsReequip then
				...
				if srcContainer:getType() == "floor" then
					P4TidyUpMeister.setFloorItem(P4TidyUpMeister.temp.floorItems, P4TidyUpMeister.temp.worldRotations, item)
				end
			end
		end
	end
...
P4TidyUpMeister.evacuate = function()
	...
	P4TidyUpMeister.temp.inventoryItems = P4TidyUpMeister.inventoryItems
...
P4TidyUpMeister.revert = function()
	...
	P4TidyUpMeister.inventoryItems = P4TidyUpMeister.temp.inventoryItems
...
P4TidyUpMeister.reequip = function(player)
	if P4TidyUpMeister.shouldSkipForBicycle(player) then
		P4TidyUpMeister.reset()
		return
	end
	...
	if P4TidyUpMeister.needsReequip and not P4TidyUpMeister.doNotReequip then
		...
		if P4TidyUpMeister.primaryItem then
			...
			ISTimedActionQueue.add(P4RestoreAction:new(player, P4TidyUpMeister.primaryItem, P4TidyUpMeister.isPrimaryActivated))
		else
			if primaryItem then
		...
		if P4TidyUpMeister.secondaryItem then
			if not bTwoHands then
				...
				ISTimedActionQueue.add(P4RestoreAction:new(player, P4TidyUpMeister.secondaryItem, P4TidyUpMeister.isSecondaryActivated))
			end
		else
		...
		for i=0, inventoryItems:size()-1 do
			local inventoryItem = inventoryItems:get(i)
			if P4TidyUpMeister.inventoryItems[inventoryItem] == nil then
				table.insert(addedItems, inventoryItem)
			end
		end
		for item,containerInfo in pairs(P4TidyUpMeister.transferredItems) do
			...
			if needsTransfer then
				...
				local contains = inventory:contains(item)
				if isMultiplayer() then
					local inventoryItems = inventory:getItems()
					for i=0, inventoryItems:size()-1 do
						local inventoryItem = inventoryItems:get(i)
						if inventoryItem:getType() == item:getType() then
							contains = true
							break
						end
					end
				...
				if contains then
					...
					for _,replaceItem in pairs(replaceItems) do
						for _,addedItem in ipairs(addedItems) do
							if replaceItem == addedItem:getFullType() or replaceItem == addedItem:getType() then
								local action = ISInventoryTransferAction:new(player, addedItem, inventory, destContainer)
								if gridInfo then
									action.setTetrisTarget(action, gridInfo.x, gridInfo.y, gridInfo.gridIndex, gridInfo.isRotated)
								end
								ISTimedActionQueue.add(action)
